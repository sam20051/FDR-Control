...
For example, if we have 1000 tests and want to maintain an overall α of 0.05, the Bonferroni threshold would be 0.00005, which is extremely lenient and often leads to very few discoveries, even when there are true effects present.
...
The figure below illustrates the difference between FWER and FDR thresholds. The dashed line represents the BH threshold for FDR control, which allows for more discoveries while controlling the expected proportion of false positives. The dotted line represents the Bonferroni threshold for FWER control, which is much lower and results in fewer discoveries.
...
plt.xlabel("Rank")
plt.ylabel("p-value")
plt.title("FWER vs FDR Thresholds (with strong signals)")
plt.legend()
plt.show()
...
plt.xlim(0, 100)  # first 100 ranks
plt.ylim(0, 0.01)  # very small p-values

plt.xlabel("Rank")
plt.ylabel("p-value")
plt.title("Zoomed View: Small p-values and Thresholds")
plt.legend()
plt.show()
```
